...
export const siteConfig = {
  "personal": {
    ...
    "experience": "3+",
    "cvPath": "Professional CV - Ashraful Alam Ashik.pdf",
    "profileImgPath": "profile.png",
    "socials": [
      {
        "platform": "WhatsApp",
        "url": "http://wa.me/+8801737940250",
        "icon": "MessageCircle"
      },
      {
        "platform": "LinkedIn",
  ...
  "education": [
    {
      ...
      "school": "Government Sherpur College, Bogura",
      "year": "Expected 2026"
    },
    {
      "degree": "HSC, Business Studies",
      "school": "Bogra Government College",
      "year": "2020 | GPA: 4.50"
    }
  ],
  "services": [
  ...
  "caseStudies": [
    ...
    },
    {
      ...
      "problem": "Citizen Development Society (CDS)-এর জন্য একটি মেম্বারশিপ ফর্ম প্রয়োজন ছিল যা সাধারণ নাগরিকরা সহজে ব্যবহার করতে পারেন। প্রধান সমস্যা ছিল বাংলা/ইংরেজি ভাষার সামঞ্জস্য রাখা, একই মোবাইল নম্বর দিয়ে ডুপ্লিকেট সাবমিশন রোধ করা এবং প্রিন্ট-যোগ্য অফিসিয়াল ফরম্যাটের পিডিএফ তৈরি করা।",
      "solution": "একটি সার্ভারলেস ফ্রন্টএন্ড সলিউশন যা ফায়ারবেস ফায়ারস্টোর ডেটাবেস এবং একটি কাস্টম লাইটওয়েট জেএস ট্রান্সলেশন ইঞ্জিনের সাহায্যে তৈরি। এতে ডুপ্লিকেট সাবমিশন ঠেকাতে ফায়ারস্টোরে `phone_index` ইউনিক ব্যাচ রাইট ট্রানজেকশন এবং html2pdf.js দিয়ে অন-দ্য-ফ্লাই ডক জেনারেশন ব্যবহার করা হয়েছে।",
      "results": [
        "১০০% ডুপ্লিকেট সাবমিশন প্রতিরোধ (Firestore Rules + Batch Writes)",
        "সম্পূর্ণ ক্লায়েন্ট-সাইড দ্বিভাষিক (BN/EN) ইঞ্জিন",
      ...
      "summary": "CDS Membership Portal হল Citizen Development Society (CDS)-এর জন্য নির্মিত একটি মেম্বারশিপ অ্যাপ্লিকেশন সিস্টেম। এতে কোনো থার্ড-পার্টি লাইব্রেরি ছাড়াই জিরো-ডিপেন্ডেন্সি ক্লায়েন্ট-সাইড ট্রান্সলেশন ইঞ্জিন ও ফায়ারবেস সিকিউরিটি দিয়ে নির্ভুল ও নিরাপদ মেম্বারশিপ তালিকা পরিচালনা করা যায়।",
      "challenge": {
        "description": "ফর্ম সাবমিশনে নিরাপত্তা নিশ্চিত করা এবং ডুপ্লিকেট ফোন নম্বর দিয়ে মাল্টিপল সাবমিশন রোধ করা অত্যন্ত জরুরি ছিল। এছাড়া, ব্যবহারকারী যে ভাষায় (বাংলা অথবা ইংরেজি) ফর্মটি পূরণ করবে, রসিদ বা পিডিএফটি যেন হুবহু সেই ভাষাতেই জেনারেট হয় তা নিশ্চিত করা একটি বড় চ্যালেঞ্জ ছিল।",
        "objectives": [
          "ফায়ারবেস সিকিউরিটি রুলস ও ডুয়াল-কালেকশন ব্যাচ রাইটসের সাহায্যে ফোন নম্বরের ইউনিকনেস চেক করা।",
          "কাস্টম রিকার্সিভ ডম ট্রাভার্সালের মাধ্যমে কোনো লাইব্রেরি ছাড়া তাৎক্ষণিক পেজ ট্রান্সলেশন এনফোর্স করা।",
          "html2pdf.js দিয়ে রিয়েল-টাইমে ইনপুট ভ্যালুর বদলে টেক্সট রিড করে সঠিক পিডিএফ তৈরি করা।"
        ]
      },
      "architectureAndSecurity": {
        "title": "Firestore Security Rules এবং ডুপ্লিকেট প্রতিরোধ",
        "points": [
          {
            "title": "Firestore Batch Transactions ও ডুপ্লিকেশন লক",
            "description": "ফোন নম্বরের ডুপ্লিকেশন ঠেকাতে একটি সেকেন্ডারি কালেকশন phone_index ব্যবহার করা হয়েছে। সাবমিশনের সময় applications এবং phone_index-এ একটি পারমাণবিক (atomic) ব্যাচ রাইট করা হয়। ফোন নম্বরটি ইউনিক না হলে ফায়ারবেস ডাটাবেস রুলস ট্রানজেকশনটি সরাসরি রিজেক্ট করে দেয়।",
            "codeSnippet": "// Firestore Batch Write and unique check\nconst batch = writeBatch(db);\nbatch.set(doc(db, \"applications\", appId), appData);\nbatch.set(doc(db, \"phone_index\", phone), { appId: appId });\nawait batch.commit();"
          },
          {
            "title": "অন-দ্য-ফ্লাই পিডিএফ ডাটা ম্যাপিং",
            "description": "সিলেক্টেড ড্রপডাউন ভ্যালুর বদলে ডাইনামিকালি ডম এলিমেন্ট থেকে সিলেক্টেড টেক্সট (.text) রিড করার লজিক ইমপ্লিমেন্ট করা হয়েছে, যার ফলে ট্রান্সলেটেড পিডিএফ-এ সঠিক ইংরেজি বা বাংলা জেলা ও উপজেলা নাম প্রিন্ট হয়।"
          },
          {
            "title": "ক্লায়েন্ট-সাইড ইমেজ ফিল্টারিং ও কম্প্রেশন",
            "description": "নিরাপত্তা নিশ্চিতে ফর্মের সাথে সংযুক্ত প্রোফাইল ছবি ও সিগনেচার আপলোডে কঠোরভাবে শুধুমাত্র ইমেজ ফাইল ভ্যালিডেশন এবং আপলোডের আগে ক্লায়েন্ট-সাইডে জেএস দিয়ে ইমেজ সাইজ কম্প্রেশন করা হয়।"
          }
        ]
      },
      "keyFeatures": [
        {
          "title": "দ্বিভাষিক বাংলা/ইংরেজি ইন্টারফেস",
          "description": "তাৎক্ষণিকভাবে পুরো পেজের হেডার, লেবেল, ড্রপডাউন ও এরর মেসেজ বাংলা ও ইংরেজিতে কনভার্ট করার সুবিধা।"
        },
        {
          "title": "অটো-ফিল ও স্মার্ট ফর্ম আইডি",
          "description": "প্রেজেন্ট এবং পার্মানেন্ট অ্যাড্রেস একই হলে এক ক্লিকে সম্পূর্ণ অ্যাড্রেস কপি করার স্মার্ট লজিক এবং ইউনিক আইডি জেনারেশন।"
        },
        {
          "title": "ফায়ারস্টোর সিকিউরিটি রুলস",
          "description": "পাবলিক ইউজারদের জন্য শুধুমাত্র ক্রিয়েট পারমিশন দেওয়া এবং কোনো ডাটা রিড, আপডেট বা ডিলিট করার অ্যাক্সেস ব্লক করে রাখা।"
        }
      ],
      "outcomes": [
        "১০০% নিরাপদ এবং সফল মেম্বারশিপ সাবমিশন সিস্টেম।",
        "ম্যানুয়াল ডেটা এন্ট্রি ও পেপারওয়ার্কের সময় ৯০% হ্রাস।",
  ...
  "siteIdentity": {
    "title": "Ashraful Alam Ashik | Digital Operations Manager & Full-Stack Web Developer",
    "description": "Dynamic Digital Operations Manager and Full-Stack Web Developer with 3+ years of proven expertise in SEO optimization.",
    "faviconPath": "profile.png",
    "logoType": "image",
    "logoImgPath": "profile.png"
  }
};
